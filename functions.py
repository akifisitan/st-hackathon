...
def estimate_future_expenses(user_data, expenses_data):
    ...
    for i, category in enumerate(categories, start=1):
        ...
        model = ExponentialSmoothing(
            category_expenses_array, trend="add", seasonal=None
        )
        fit = model.fit()
        ...
        inflation_rate = 0.03
        future_expenses = [
            expense * ((1 + inflation_rate) ** month)
            for month, expense in enumerate(future_expenses)
        ]
    ...
    current_month = int(
        expenses_data[-1].split(",")[0]
    )  # Assuming the last entry is the current month
